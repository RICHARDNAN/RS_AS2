...
Proportional-integral-derivative (PID) control is the most commonly used control algorithm in the industry and has become universally accepted in industrial control. \cite{picard2006toward} For autonomous robotic systems that can travel, PID control is an important aspect as it provides smoother control of the robot's motion, this will be achieved at two levels within the system: effective control of wheel speed and effective control of the robot's steering. In both cases, we want the robot to move smoothly rather than make sudden or large changes. PID control uses a continuous function to calculate the motor speed, so it is possible to respond to the motor at a fixed refresh rate (using micros) to smoothly control the speed of both wheels. It is noted that different refresh frequencies have a significant impact on the performance of the PID controller and if an optimum frequency domain is found to improve the PID performance, this will provide a good reference for future commissioning of other control systems to obtain the best response.

A plot of PID performance versus settling time is shown in Figure 1. The overshoot is the amount by which the process variable exceeds the final value, and the stabilisation time is the time it takes for the process variable to stabilise to a certain percentage of the final value (typically 5\% is used). The steady-state error is the final difference between the process variable and the set point. \cite{picard2022toward}It is important to choose one or more of these variables to define the performance of the system, which shows whether the controller system can still meet the requirements for smooth robot operation in the worst case of one independent variable, the PID performance. We use overshoot and stabilisation time to measure the performance of the PID controller. Keeping the overshoot within a certain range for different update frequencies and a shorter stabilisation time will indicate higher  performance. More details on PID performance are described in subsequent sections.
% 插入系统框图
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.6\linewidth]{Report/Pic/PID.png}}
\caption{PID performance versus settling time.}
\label{fig1}
\end{figure}

\subsection{Hypothesis Statement}

An experiment will be designed to find the link between the refresh frequency and the performance of a PID. Although the integral control will filter out higher frequency sensor noise, it is slower to respond to current errors\cite{picard2006toward}, so we assume that:

\begin{itemize}
\item A high update frequency makes changes in the PID controller control quantity not reflected in the controlled object promptly and reduces performance.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width = 300]{Report/Pic/WheelResponse.png}}
\caption{System Components.}
\label{fig1}
\end{figure}
