...
Proportional-integral-derivative (PID) control is the most commonly used control algorithm in the industry and has become universally accepted in industrial control. [1] For autonomous robotic systems that can travel, PID control is an important aspect as it provides smoother control of the robot's motion, this will be achieved at two levels within the system: effective control of wheel speed and effective control of the robot's steering. In both cases, we want the robot to move smoothly rather than make sudden or large changes. PID control uses a continuous function to calculate the motor speed, so it is possible to respond to the motor at a fixed refresh rate (using micros) to smoothly control the speed of both wheels. It is noted that different refresh frequencies have a significant impact on the performance of the PID controller and if an optimum frequency domain is found to improve the PID performance, this will provide a good reference for future commissioning of other control systems to obtain the best response.

A plot of PID performance versus regulation time is shown in Figure 1. The overshoot is the amount by which the process variable exceeds the final value, and the stabilisation time is the time it takes for the process variable to stabilise to a certain percentage of the final value (typically 5\% is used). The steady-state error is the final difference between the process variable and the set point. [2] It is important to choose one or more of these variables to define the performance of the system, which shows whether the controller system can still meet the requirements for smooth robot operation in the worst case of one independent variable, the PID performance. We use overshoot and stabilisation time to measure the performance of the PID controller. Keeping the overshoot within a certain range for different update frequencies and a shorter stabilisation time will indicate higher  performance. More details on PID performance are described in subsequent sections.
\subsection{Hypothesis Statement}

An experiment will be designed to find the link between the refresh frequency and the performance of a PID. Since sensor detection speed is limited by hardware such as wires and motor performance, the response speed will be limited, so we assume that:

\begin{itemize}
\item A high refresh frequency makes changes in the PID controller control quantity not reflected in the controlled object promptly and reduces performance.
\end{itemize}


We aim to find the optimum update frequency to exploit the performance of the PID, so we also assume that:

\begin{itemize}
\item A low update frequency results in a low response that does not achieve the control objective promptly and degrades the performance.
\item There should be an optimal frequency band in the frequency domain that better balances all aspects of the PID and achieves better performance.  
\end{itemize}
...
System has 4 main components: PID controller, Motor Driver, Wheel, Encoder, Kinematics.
\begin{itemize}
    \item\textbf{PID Controller: }We use basic PID controller in our system to minimize the number of irrelevant variables. We don't want common improvements for PID to be variables in the system, like integration limitation, etc. The PID controller we use can be described as below:
        ...
        \par
    \item\textbf{Wheel and Motor Driver: }For Pololu 3pi+ 32U4 robot, user could control the motor speed by controlling motor PWM duty cycle ratio. Motor takes 0~255(integer) as input and output 0~5V power to the motor.
    %插入电机速度与输入的特性图 
    \item\textbf{Encoder: }On each wheel shaft we have a incremental encoder, with that we could measure wheel movements. Encoder counts between 358~359 for a 360° wheel spin
